...
final readonly class EgalineCalculatorMetabox
{
    ...
    public function renderMetaboxFields(): void
    {
        ...
        $metaValues = [
            'enable_calculator'   => $this->getMetaValue((int) $post->ID, self::META_KEYS['enable'], $this->getDefaultSetting('default_enable_calculator', false) ? 'yes' : 'no'),
            'kg_per_bag'          => $this->getMetaValue((int) $post->ID, self::META_KEYS['kg_bag'], $this->getDefaultSetting('default_kg_per_bag', 1.0)),
            'kg_per_mm'           => $this->getMetaValue((int) $post->ID, self::META_KEYS['kg_mm'], $this->getDefaultSetting('default_kg_per_mm', '')),
            'kg_per_m2'           => $this->getMetaValue((int) $post->ID, self::META_KEYS['kg_m2'], $this->getDefaultSetting('default_kg_per_m2', '')),
            'calculation_mode'    => $this->getMetaValue((int) $post->ID, self::META_KEYS['mode'], self::CALCULATION_MODES['PER_MM']),
            'discount_threshold'  => $this->getMetaValue((int) $post->ID, self::META_KEYS['discount_threshold'], $this->getDefaultSetting('default_discount_threshold', '')),
            'discount_percentage' => $this->getMetaValue((int) $post->ID, self::META_KEYS['discount_percentage'], $this->getDefaultSetting('default_discount_percentage', '')),
        ];
    ...
    private function getDefaultSetting(string $key, mixed $default): mixed
    {
        $settings = get_option('egaline_calculator_settings', []);
