...
jQuery(document).ready(($) => {
  const publicSettings = window.egalineCalculatorSettings || {};
  const config = {
    defaultKgPerBag: 15,
    showDiscountText: publicSettings.showDiscountText !== false,
    defaultDarkMode: publicSettings.defaultDarkMode === true,
    labelMm: publicSettings.labelMm || "Hoe dik egaliseren in mm?",
    labelM2: publicSettings.labelM2 || "Aantal m² egaliseren?",
    labelBags: publicSettings.labelBags || "Aantal zakken",
    roundUpBags: publicSettings.roundUpBags !== false,
    minBags: parseInt(publicSettings.minBags, 10) || 1,
    maxBags: parseInt(publicSettings.maxBags, 10) || 100000,
    maxM2: parseFloat(publicSettings.maxM2) || 100000,
    maxMm: parseFloat(publicSettings.maxMm) || 100000,
    priceDecimals: Math.min(4, Math.max(0, parseInt(publicSettings.priceDecimals, 10) || 2)),
    requireValidCalculation: publicSettings.requireValidCalculation !== false,
  };
  ...
  const showCalcError = (calculator, message) => {
    ...
    $msg.prop("hidden", false).text(message).show();
    setTimeout(() => {
      $msg.fadeOut();
    }, 5000);
  };
  ...
  const initializeCalculator = (calculator) => {
    ...
    const formatMoney = (value) => Number(value || 0).toFixed(config.priceDecimals);

    // Update labels volgens rekenmodus
    const updateLabel = () => {
      $inputs.labelEgalineMm.text(config.labelMm);
      $inputs.labelEgalineM2.text(config.labelM2);
      $inputs.labelResultBags.text(`${config.labelBags} (${BAG_WEIGHT}kg)`);
    };
    ...
    const updateResults = () => {
      ...
      const rawBags = BAG_WEIGHT > 0 ? totalKg / BAG_WEIGHT : 0;
      const computedBags = clamp(config.roundUpBags ? Math.ceil(rawBags) : Math.round(rawBags), config.minBags, config.maxBags);
      ...
      if (multipliedBags >= discountThreshold && discountPercentage > 0) {
        ...
        $inputs.totalPrice.text(config.showDiscountText ? `Korting: ${discountPercentage}% (${formatMoney(discountedTotal)}) EUR` : `${formatMoney(discountedTotal)} EUR`);
      } else {
        $inputs.totalPrice.text(`${formatMoney(multipliedOriginalTotal)} EUR`);
      }
    ...
    $inputs.qtyBtns.off("click.qty").on("click.qty", (e) => {
      ...
      let newVal = isPlus ? currentVal + step : Math.max(0, currentVal - step);
      if ($input.hasClass("egaline-mm")) newVal = clamp(newVal, 0, config.maxMm);
      if ($input.hasClass("egaline-m2")) newVal = clamp(newVal, 0, config.maxM2);
      if ($input.hasClass("result-bags")) newVal = clamp(newVal, config.minBags, config.maxBags);
    ...
    if (config.defaultDarkMode) {
      calculator.addClass("dark-mode");
    }
  ...
  const loadCalculatorData = () => {
    try {
      ...
      if (window.localStorage.getItem(darkModeStorageKey) === "1" || config.defaultDarkMode) {
  ...
  $("form.cart")
    ...
    .on("submit.calculatorSubmit", function () {
      ...
      if (config.requireValidCalculation && (thickness <= 0 || area <= 0 || bags <= 0 || thickness > config.maxMm || area > config.maxM2 || bags > config.maxBags)) {
  ...
  if ($variationsForm.length) {
    $variationsForm.on("woocommerce_variations_loaded", () => {
      ...
      if (variations && variations.length > 0) {
        ...
        if (defaultVariation && defaultVariation.display_price) {
          const cleanPrice = String(defaultVariation.display_price);
          $(".egaline-calculator").each(function () {
            $(this).data("variation-price", defaultVariation.display_price);
            $(this).find(".calc-variation-price").prop("hidden", false).text("(" + cleanPrice + ")").show();
          });
        }
      }
    });
    ...
    $variationsForm.on("found_variation", (event, variation) => {
      if (variation.display_price) {
        const cleanPrice = String(variation.display_price);
        $(".egaline-calculator").each(function () {
        ...
        $(".egaline-calculator-warning").hide();
      }
    });
  }
  ...
  $variationsForm.on("reset_data", () => {
    ...
    $(".egaline-calculator-warning").show();
  });
});
